...
static void temp_task(void *param) {
    /* Task entry log */
    ESP_LOGI(TAG, "temp task has been started!");

    /* Loop forever */
    while (1) {
        /* Update temp value every 1 second */
        update_temp();
        ESP_LOGI(TAG, "temp updated to %d", get_temp());

        /* Send temp indication if enabled */
...
void app_main(void) {
    ...
    rc = xTaskCreate(temp_task, "Temp", 4 * 1024, NULL, 5, NULL);
    if (rc != pdPASS) {
