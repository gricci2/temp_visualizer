...
static const ble_uuid128_t temp_svc_uuid = BLE_UUID128_INIT(
        0x10, 0x32, 0x54, 0x76,
        0x98, 0xBA,
        0xDC, 0xFE,
        0x01, 0x23,
...
static bool temp_notify_status = false;
...
error:
    ESP_LOGE(
        TAG,
        "unexpected access operation to temp characteristic, opcode: %d",
        ctxt->op);
    return BLE_ATT_ERR_UNLIKELY;
}
...
void send_temp_notification(void) {
    if (temp_notify_status && temp_chr_conn_handle_inited) {
        temp_chr_val = get_temp();
        ble_gatts_notify(temp_chr_conn_handle,
                           temp_chr_val_handle);
        ESP_LOGI(TAG, "temp notification sent!");
    }
}
...
// }
void gatt_svr_reset_temp_subscription(void) {
    temp_chr_conn_handle = BLE_HS_CONN_HANDLE_NONE;
    temp_chr_conn_handle_inited = false;
    temp_notify_status = false;
}
